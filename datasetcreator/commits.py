...
def files_in_commits(dataFolderPath, user_name):
    '''
...
def commit_changes(dataFolderPath, user_name):
    '''
    ...
    for item in fin_dict.keys():        
        try:
            language_commits[dbs.languages_db(item)] = fin_dict[item]
        except KeyError:
            continue
        
    return language_commits

def empty_commit_message(dataFolderPath, user_name):
...
def bug_fixing_contribution(dataFolderPath, user_name):
    '''
    ...
    for key in commit_authored.keys():
        if any(word in commit_authored[key]["commit"]["message"] for word in dbs.keywords_db()[1]):
