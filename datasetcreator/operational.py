...
def documentation_commit(dataFolderPath, user_name):
    ...
    for key in commit_authored.keys():
        for item in range(len(commit_authored[key]["files"])):
            ...
            if language in dbs.keywords_db()[4]:
...
def documentation_comments(dataFolderPath, user_name):
    '''
    ...
    for issue_id in comments["comments_on_issues"].keys():
        for comment_id in comments["comments_on_issues"][issue_id].keys():
            if any(word in comments["comments_on_issues"][issue_id][comment_id]["body"] for word in dbs.keywords_db()[4]):
    ...
    for issue_id in comments["commnents_on_committs"].keys():
        for comment_id in comments["commnents_on_committs"][issue_id].keys():
            if any(word in comments["commnents_on_committs"][issue_id][comment_id]["body"] for word in dbs.keywords_db()[4]):
                documentation_comments_count += 1
